...
function saveDraft() {
    ...
    if (existingIndex >= 0) {
        createOverwriteModal(projectName, () => {
            ...
            showNotification(`Draft "${escapeHtml(projectName)}" saved successfully!`, 'success');
            ...
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title">
                        <i class="bi bi-exclamation-triangle me-2"></i>Draft Exists
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>Draft "<strong>${escapeHtml(projectName)}</strong>" already exists.</p>
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header bg-info text-white">
                    <h5 class="modal-title">
                        <i class="bi bi-file-text me-2"></i>${escapeHtml(draft.name)} - Preview
                    </h5>
                        ...
                        <div class="col-md-6">
                            ...
                            <table class="table table-sm">
                                ${Object.entries(draft.projectInfo).map(([key, value]) =>
                                    value ? `<tr><td><strong>${escapeHtml(key.replace(/_/g, ' '))}:</strong></td><td>${escapeHtml(value)}</td></tr>` : ''
                                ).join('')}
                            </table>
                        </div>
                        <div class="col-md-6">
                    ...
                    ${draft.categories.map(category => `
                        <div class="card mb-3">
                            <div class="card-header">
                                <h6 class="mb-0">${escapeHtml(category.name)}</h6>
                            </div>
                            <div class="card-body">
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">
                        <i class="bi bi-folder2-open me-2"></i>Saved Drafts
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                            ...
                            <tbody id="draftsTableBody">
                                ${drafts.map((draft, index) => `
                                    <tr data-draft-id="${draft.id}">
                                        <td><strong>${escapeHtml(draft.name)}</strong></td>
                                        <td>${escapeHtml(draft.projectInfo.client_name || 'N/A')}</td>
...
// ---------------------------------------------------------------------------
// Load a selected draft into the form
// ---------------------------------------------------------------------------
...
function loadSelectedDraft(draftId) {
    ...
    showNotification(`Draft "${escapeHtml(draft.name)}" loaded successfully!`, 'success');
}
...
function performDelete(draftId) {
    ...
    if (draftIndex >= 0) {
        ...
        showNotification(`Draft "${escapeHtml(draftName)}" deleted!`, 'warning');
...
// ---------------------------------------------------------------------------
// Notification helper (mirrors core.js for self-containment)
// ---------------------------------------------------------------------------
